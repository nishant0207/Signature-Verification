...
TEST_PATH = "data/origin"

# Load filenames
genuine_files = listdir(GENUINE_PATH)
forged_files = listdir(FORGED_PATH)

print(f"Total Genuine Signatures: {len(genuine_files)}")
print(f"Total Forged Signatures: {len(forged_files)}")

# Feature extraction function
def extract_features(image_path):
    """Extracts features from an image for SVM training."""
    try:
        preprocessed_image = preproc.preproc(image_path, display=False)
        if preprocessed_image is None:
            print(f"Skipping {image_path}: Preprocessing returned None")
            return None
        ...
        if features_result is None:
            print(f"Skipping {image_path}: get_contour_features returned None")
            return None
        ...
        return [
            aspect_ratio, convex_hull_area / bounding_rect_area, contours_area / bounding_rect_area, ratio,
...
for img in genuine_files:
    feature_values = extract_features(f"{GENUINE_PATH}/{img}")
    if feature_values:
        X.append(feature_values)
        y.append(1)  # Genuine label

for img in forged_files:
    feature_values = extract_features(f"{FORGED_PATH}/{img}")
    if feature_values:
        X.append(feature_values)
        y.append(0)  # Forged label
...
print(f"Final SVM Accuracy: {accuracy:.2f}")
